...
- 祝日月水金の `Aフ` 例外割当（技師長/パート/時短）
- 週40時間上限・月上限（非パート）
- 月休日数下限（2026ルール）
- パート週2回原則、必要時のみ週3回まで許容
- 生成不能箇所の可視化:
  - 必須枠不足（不足アラート）
...
- `対象月` を入力すると、その月の日本祝日がGUIに初期反映されます。必要なら祝日モードで手修正できます。
...
制約メモ:
- 技師長・パート・時短は通常 `Aフ/B/C/Dフ/Eフ/Fフ` には入りませんが、月水金の祝日だけ `Aフ` に例外配置されることがあります。
- パートは週2回が原則で、必須枠不足を避けるために必要な場合のみ週3回になることがあります。
